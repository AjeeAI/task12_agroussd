...
workspace = Path(f"task12_agroussd\agro_ussd\data")
workspace.mkdir(exist_ok=True)
csv_file = Path.cwd()/workspace/"users.csv"
def save_user(path,user_credential):
    ...
    if path.exists()==True:
        ...
        with open(path,"w",newline="", encoding="utf-8") as f:
            ...
            writer.writeheader()
            writer.writerow(login_credential)
    print(f"You have successfully created an account {user_name}\nYour Username is {user_Id}\nPLEASE DONT FORGET YOUR PASSWORD")
    return "SUCCESS"
...
if acc_status == "2":
    ...
    signup_status= save_user(csv_file,login_credential)
